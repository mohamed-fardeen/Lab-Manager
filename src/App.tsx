              ...
              <div className="flex flex-col md:flex-row gap-4 mb-6">
                ...
                <div className="flex gap-3">
                   <select 
                     ...
                     className="bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-electric-blue/50 text-slate-300"
                   >
                   ...
                   >
                     ...
                     <option value="record">Record (PDF)</option>
                     <option value="program">Program</option>
                     <option value="screenshot">Screenshot</option>
                   </select>
                </div>
              </div>
              {/* Smart Search Results View */}
              {(searchQuery || searchLanguage || searchType) ? (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold flex items-center gap-2 text-electric-blue">
                      <Search size={20} />
                      {isSearching ? 'Analyzing Records...' : `Search Results (${searchResults.length})`}
                    </h2>
                    <Button 
                      variant="ghost" 
                      size="sm" 
                  ...
                  {searchResults.length === 0 && !isSearching ? (
                    <div className="glass-panel p-12 flex flex-col items-center justify-center text-center gap-4">
              ...
              <div className="space-y-8">
                ...
                {!loading && allFiles.length > 0 && (() => {
                  const filtered = allFiles.filter(f => {
                    ...
                    const parentFolder = folder?.parent_id ? folders.find(p => p.id === folder.parent_id) : folder;
                  ...
                  return ['Today', 'Yesterday', 'Older Intelligence'].map(groupName => {
                ...
                {allFiles.length === 0 && (
                  <div className="text-center py-20 text-slate-600 italic text-sm border border-dashed border-slate-800 rounded-3xl">
                    No intelligence sequencing logs found in the current timeframe.
                  </div>
                )}
              </div>
            </div>
